...
// Incorporates parameter counts, dimensions, special layers (conv, maxpooling,
// etc.) and a frequency table for activation functions
...
private:
    ...
    int** layer_dims;
    // 2D array of frequencies (e.g. layer [23] can have [5 ReLU's, 2 sigmoids,
7 binary steps and 0 for other functions])
    // If within a layer the total # of specified functions != the # of
perceptrons, there will be a proportional allocation unsigned int**
actv_func_frequencies_table;
    // Parameters of funcs which use them (currently only would support a single
value for a type of function in a layer) unsigned int** actv_func_params_table;
}; */
...
// Transition class for loading in/saving model, includes parameters and
// activation func ID's (enum <-> func pointer mappings?)
